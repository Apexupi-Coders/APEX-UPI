...
public class NpciCallbackController {
    ...
    @PostMapping(value = "/RespCredit/{txnId}", consumes = MediaType.APPLICATION_XML_VALUE, produces = MediaType.APPLICATION_XML_VALUE)
    public String respCredit(@PathVariable String txnId, @RequestBody String rawXml) {
        return callbackCorrelationService.processCallback(txnId, rawXml);
    }
}
