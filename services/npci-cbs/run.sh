...
echo "║  NPCI · PSP · CBS · Dashboard · Webhook                  ║"
echo "╚══════════════════════════════════════════════════════════╝"
echo ""
...
echo "│  Webhook Service  │ http://localhost:8085    │ 8085         │"
...
echo ""
echo "  # Register a webhook (use https://webhook.site for a free test URL):"
echo "  curl -X POST http://localhost:8085/api/webhooks/register \\"
echo "    -H 'Content-Type: application/json' \\"
echo "    -d '{\"targetUrl\":\"https://webhook.site/your-id\",\"secret\":\"mysecret\",\"subscribedEvents\":[\"SUCCESS\",\"REVERSED\"],\"owner\":\"GPay-PSP\"}'"
echo ""
echo "  # View all registered webhooks:"
echo "  curl http://localhost:8085/api/webhooks"
echo ""
echo "  # View delivery logs for a registration:"
echo "  curl http://localhost:8085/api/webhooks/{id}/logs"
echo ""
echo "  # Logs:"
echo "  docker-compose logs -f npci-router      # NPCI adapter + auto-reversal"
echo "  docker-compose logs -f cbs-service      # dual DB writes"
echo "  docker-compose logs -f webhook-service  # webhook deliveries"
echo ""
